...
$addUrl=$GLOBALS['BRAND_URL']."?page=".PG_ADMIN_ADD_STORAGE;
if (SID!='')
	$addUrl.="&".SID;
$addBtn="<a target='$target' href=\"$addUrl\"><img src=\"$addIcon\"> ${gText['M_ADD_HOSTING']}</a>";

$editUrl=$GLOBALS['BRAND_URL']."?page=".PG_ADMIN_EDIT_STORAGE;
if (SID!='')
	$editUrl.="&".SID;
...
$deleteUrl=VM_API."?cmd=DELETE_STORAGE&return=".$retPage."&brand=".$GLOBALS['BRAND_NAME']."&brandUrl=".$GLOBALS['BRAND_URL'];
if (SID!='')
	$deleteUrl.="&".SID;
...
$errMsg=VObject::SelectAll(TB_STORAGESERVER, $query, $result);
if ($errMsg!='') {
	ShowError($errMsg);
	return;
} 
